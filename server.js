...
app.get("/", (req, res) =>{
    if (req.user){
        const postsStatement = db.prepare("SELECT * FROM post WHERE authorid = ? ORDER BY createDate DESC")
...
function sharedPostValidation(req){
    ...
    if (!req.body.title) errors.push("You myst provide a title")
    if (!req.body.body) errors.push("You myst provide content")
...
app.post("/edit-post/:id",mustBeLoggedIn, (req, res)=>{
    ...
    //if you're not the author, redirect ot homepage
    if (post.authorid !== req.user.userid) {
        return res.redirect("/")
    }
    ...
    const updateStatement = db.prepare("UPDATE post SET title = ?, body = ? WHERE id = ?")
...
app.get("/post/:id", (req, res)=>{
    const statement = db.prepare("SELECT post.*, users.username FROM post INNER JOIN users ON post.authorid = users.id WHERE post.id= ?")
    const post = statement.get(req.params.id)
...
app.post("/create-post",mustBeLoggedIn, (req, res)=>{
    ...
    //save into database\
    const ourStatement = db.prepare("INSERT INTO post (title, body, authorid, createDate) VALUES (?, ?, ?, ?)")
...
app.post("/register", (req,res)=>{
    ...
    if (req.body.password && req.body.password.length < 8) errors.push("Username must be at least 8 characters")
    if (req.body.password && req.body.password.length > 30) errors.push("Username cannot exceed 30 characters")
